...
void generate_answers(vector <vector <int> > &guesses, vector <int> &blacks, vector <int> &whites, vector <int> &current_answer, int current, vector <vector <int> > &possibles){
    // guesses is a list of guesses made so far (each element of guesses is an integer list of 4 elements)
    // blacks is a list of integers that has the same number of elements as guesses, blacks[i] represents the number of blacks in guesses[i]
    // whites is a list of integers that has the same number of elements as guesses, whites[i] represents the number of whites in guesses[i]
    // current_answer represents the current answer created from guesses so far in the current call
    // current represents the index of the guess we are analysing in the current call
    // possibles holds all the possible answers we can get from these guesses, it gets filled up gradually
    int i;

    if (current >= guesses.size()){
        bool sw = false;
        for (i = 0; i<possibles.size(); i++){
            if (current_answer == possibles[i]){
                sw = true;
                break;
            }
        }
...
// void eliminate_answers(vector <int> &colors,){}
...
void play_computerVScomputer(int num_color,int num_pick, vector <vector <int> > &guesses, 
    ...
    // choosing the target
    // create_arrangement(num_color, num_pick, arrangement, useless_colors);

    //we may have to delete later
    ...
    // do {
    //     // print_guess(guesses[num_guesses-1],blacks[num_guesses-1],whites[num_guesses-1],colors);
    // } while (blacks[num_guesses - 1] == 0 && whites[num_guesses - 1]);
    ...
    for(i=0;i<arrangements.size();i++){
        if(!intersection(arrangements[i],useless_colors) && find(guesses.begin(), guesses.end(), arrangements[i]) == guesses.end()){
            ...
            a.id = ++id;
            // print_guess(arrangements[i],-1,-1,colors);
    ...
    // cout << useless_colors.size() << endl;
    // cout << num_guesses << endl;
    // cout << guesses.size() << " " << blacks.size() << " " << whites.size() << endl;
    // for(i=0;i<answers.size();i++){
    //     print_guess(answers[i],-1,-1,colors);
    // }
...
void init_colors(vector <string> &colors){
    ...
    colors.push_back("Amber");
    colors.push_back("Torqouise");
